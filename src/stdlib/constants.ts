...
export function registerConstants(env: Env): void {
  ...
  // RankKind (Table.AddRankColumn) and PercentileMode (List.Percentile). Values from the
  // reference (rankkind-type / percentilemode-type).
  def("RankKind.Competition", N(0));
  def("RankKind.Dense", N(1));
  def("RankKind.Ordinal", N(2));
  def("PercentileMode.ExcelInc", N(1));
  def("PercentileMode.ExcelExc", N(2));
  def("PercentileMode.SqlDisc", N(3));
  def("PercentileMode.SqlCont", N(4));
